...
gaps the 2026-08-02 pass left open, recorded in a new payload,
`data/ships/unknown-stats.jsonc` (5 records), so a program can tell an absence that means
"the module has no such stat" from one that means "nobody publishes it". Derivation is
this repository's own reconciliation: each entry restates a finding the 2026-08-02
revision below already made against coriolis-data (commit
...
- **Registered as unknown (2 fields, 5 records).** `powerDraw` on the four
  ...
  source; the file names them so the absence is a statement rather than a silence.
  Filling one means deleting its entry in the same change — `unknown-stats.test.ts`
  fails on a registered field that has a value.
...
- **`integrity` on the 83 non-armour records that lack it is *not* registered**, because
...
- **`cost` is deliberately not registered.** Every module without a price has no
  *published* price, so an absent `cost` is already unambiguous (README, list prices);
  there is nothing for a registry to disambiguate.
- **Scope.** The register covers fields the module record actually has. The base stats
  blueprints modify that no record carries at all (`EngineHeatRate`, `EnergyPerRegen`,
  the scanner ranges) cannot be named in it and stay in `TODO.md` §5.

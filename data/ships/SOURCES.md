...
`integrity`, 104 of 115 `powerDraw`, and **contradicts none of them**. The remainder are
records EDSY does not list at all — `Int_Hyperdrive_Size8_Class{1..5}`, the ten `*_free`
starter fittings, and a few built-ins such as `Int_DetailedSurfaceScanner_Tiny` and
`Int_DroneControl_ResourceSiphon`. Those figures were **read from the live game's own
outfitting and module panels** (2026-08-02 UTC), the same route this file already uses
for the in-game blueprint and Operations registries: no third-party registry publishes
them, so there is nothing to cite them to. Each is pinned in
`fixtures/ships/module-stats.json` so a port validates against the same numbers, and each
is independently consistent with its own family's curve — the size-8 drives extend the

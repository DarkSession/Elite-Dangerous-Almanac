...
2. **`tsup`** bundles ESM to `dist/` — one entry per public module, declarations, source maps, and the shared `data/` JSONC inlined through the `jsonc` esbuild plugin. It tree-shakes but **does not minify**: the output keeps readable identifiers and comments, so a stack trace or a file opened from `node_modules` stays useful, and minification is left to the consuming application's bundler. This is why the bundle-size figures above are measured the way §Build & Tree-Shaking Requirements says they are, rather than by reading `dist/` byte counts.

...
- **Provenance.** `data/<domain>/SOURCES.md` must record, per catalogue, the source, its immutable revision or checksum, the acquisition date, the derivation and every manual correction with the reasoning behind it — that is what `data/SNAPSHOTS.md` requires, and a date there is a fact about the _source_, not about this repository's history. Record it under the catalogue it describes, in the present tense ("`Int_Sensors_Size1_Class{1..5}` `integrity` is 36/32/40/48/44; coriolis-data's size-1 row is a verbatim copy of its size-2 row, so EDSY's figures are used"), never as a dated entry in a log at the top of the file.
...
data-file header or a `SOURCES.md`. Ask one question: _would adding a row make this
sentence false?_ If yes, delete the number. It is a maintenance bill, and it falls due on

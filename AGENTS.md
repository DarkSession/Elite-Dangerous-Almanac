...
| `npm run build`          | `copy-notices` → `tsup` → `prune-barrel-imports` → `attach-barrel-docs` → `dist/` (JSON catalogues inlined; whitespace stripped, names and syntax **not** minified — see below) |
...
2. **`tsup`** bundles ESM to `dist/` — one entry per public module, declarations, source maps, and the shared `data/` JSONC inlined through the `jsonc` esbuild plugin. It tree-shakes and strips whitespace, but **leaves identifiers and syntax alone** (`minifyWhitespace: true` with `minify`/`minifyIdentifiers`/`minifySyntax` off in `tsup.config.ts`, which carries the reasoning) — real minification is the consuming application's bundler's job, and that is why the size figures above are measured as §Build & Tree-Shaking Requirements describes rather than by reading `dist/` byte counts. Whitespace is the exception because the inlined catalogues dominate `dist/` and nobody reads their indentation: it was a quarter of the shipped JavaScript, and dropping it takes `dist/` from 2.85 MB to 1.92 MB while function names survive in stack traces and the published source maps still resolve a throw to its `src/**/*.ts` line.

...
C# examples are checked without a compiler: `scripts/wiki/dotnet-examples.mjs` reads every ```csharp fence under `dotnet/docs/` and `docs/`, and every `<code>` block in a `///` comment under `dotnet/src/` (the reference renders one as the same fence). A type or member the published surface does not carry fails `test:wiki`, and so `check`. It cannot see a call on a local, nor a misspelt type — a name it does not publish may be the BCL's — so write an example whose static entry points state the names it should check, and keep every statement valid C# — a bare member access is a TypeScript idiom, not one that compiles here.

...
- **Usability outranks tree-shaking when the two collide.** A registry lookup takes an *optional* catalogue argument that defaults to the whole registry (`getMaterialByName('iron')`), because a journal line hands you a symbol and nothing else — making the caller identify the category first was solving the library's problem with the user's time. A default is a static import, so the data cannot be tree-shaken away even when an explicit catalogue is passed; that is the accepted price. Keep it honest by measuring: materials 15 KB, micro resources 14 KB, commodities 28 KB, and `ships/modules` 290 KB minified (30 KB gzipped) — measured as the README defines it, over the shipped `dist/` files in the import graph. The module figure is the one that hurts, so its TSDoc and the README both name it and point at the escape hatch (import one category catalogue, search it with plain `Array` methods). **`astro/nebulae` is the deliberate counter-example** — `ALL_NEBULAE` is 682 KB, so its catalogue argument stays required. The rule of thumb: default to the whole registry unless doing so would cost more than the rest of the library, and say so in the module's own docs either way. Do not "fix" a default back into a required parameter without that measurement.

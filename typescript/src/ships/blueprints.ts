...
import type { BlueprintGrades, BlueprintFeature, EngineeringMaterial } from './engineering.js';
...
export function getBlueprintCost(
    ...
    if (!blueprint[String(grade)]) return null; // the target grade must exist
    const perGrade: EngineeringMaterial[][] = [];
    for (let g = currentGrade + 1; g <= grade; g++) {
        const entry = blueprint[String(g)];

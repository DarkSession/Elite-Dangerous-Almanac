...
const keyOf = (record: Record_): string => record.symbol;
const shipOf = (record: Record_): string | null | undefined => record.ship;
...
for (const [path, build] of [
    ...
    test(`${path}: a key matches whatever the casing and padding on either side`, () => {
        ...
        for (const wanted of ['int_hyperdrive_size6_class5', '  INT_HYPERDRIVE_SIZE6_CLASS5 \t']) {
            assert.equal(
                findByKey(catalogue, 'symbol', keyOf, wanted)?.symbol,
                catalogue[0]!.symbol,
            );
        }
    });
    ...
    test(`${path}: findByKey answers with the first match, as the scan it replaces did`, () => {
        ...
        assert.equal(findByKey(catalogue, 'symbol', keyOf, 'dupe')?.ship, 'first');
        assert.deepEqual(
            filterByKey(catalogue, 'symbol', keyOf, 'dupe').map((r) => r.ship),
            ['first', 'second'],
        );
    });
    ...
    test(`${path}: a record with no key is in no bucket rather than in an empty-string one`, () => {
        ...
        assert.deepEqual(
            filterByKey(catalogue, 'ship', shipOf, 'Anaconda').map((r) => r.symbol),
            ['c'],
        );
        assert.equal(findByKey(catalogue, 'ship', shipOf, ''), null);
        assert.deepEqual(filterByKey(catalogue, 'ship', shipOf, ''), []);
    });

    test(`${path}: two fields of one catalogue are answered separately`, () => {
        const catalogue = build([{ symbol: 'Anaconda', ship: 'Python' }]);
        assert.equal(findByKey(catalogue, 'symbol', keyOf, 'Anaconda')?.symbol, 'Anaconda');
        assert.equal(findByKey(catalogue, 'ship', shipOf, 'Anaconda'), null);
        assert.equal(findByKey(catalogue, 'ship', shipOf, 'Python')?.symbol, 'Anaconda');
    });
    ...
    test(`${path}: filterByKey hands back a fresh array, so one caller cannot edit the next answer`, () => {
        ...
        const first = filterByKey(catalogue, 'symbol', keyOf, 'a');
        first.length = 0;
        assert.equal(filterByKey(catalogue, 'symbol', keyOf, 'a').length, 2);
    });

    test(`${path}: an empty catalogue answers nothing rather than throwing`, () => {
        const empty = build([] as Record_[]);
        assert.equal(findByKey(empty, 'symbol', keyOf, 'a'), null);
        assert.deepEqual(filterByKey(empty, 'symbol', keyOf, 'a'), []);
    });
}
...
test('an indexed catalogue is read once; a scanned one on every call', () => {
    ...
    // Counts `keyOf` calls rather than timing anything, so it cannot flake.
    const probe = (catalogue: readonly Record_[]): number => {
        let calls = 0;
        const counting = (record: Record_): string => {
            calls++;
            return record.symbol;
        };
        findByKey(catalogue, 'symbol', counting, 'a');
        calls = 0;
        findByKey(catalogue, 'symbol', counting, 'a');
        return calls;
    };
    const records = (): Record_[] => [{ symbol: 'a' }, { symbol: 'b' }];
    assert.equal(probe(indexed(records())), 0, 'a second lookup re-reads nothing');
    assert.ok(probe(scanned(records())) > 0, 'an unindexable catalogue is re-read');
});
...
test('an unfrozen array of frozen records is never cached', () => {
    ...
    assert.equal(findByKey(growing, 'symbol', keyOf, 'b'), null);
    growing.push(Object.freeze({ symbol: 'b' }));
    assert.equal(findByKey(growing, 'symbol', keyOf, 'b')?.symbol, 'b');
    assert.equal(filterByKey(growing, 'symbol', keyOf, 'b').length, 1);
});
...
test('a frozen array of mutable records is never cached', () => {
    ...
    assert.equal(findByKey(catalogue, 'symbol', keyOf, 'Gold')?.symbol, 'Gold');

    records[0]!.symbol = 'Platinum';
    assert.equal(findByKey(catalogue, 'symbol', keyOf, 'Platinum')?.symbol, 'Platinum');
    assert.equal(findByKey(catalogue, 'symbol', keyOf, 'Gold'), null);
    ...
    assert.equal(findByKey(catalogue, 'symbol', keyOf, 'Gold')?.symbol, 'Gold');
    assert.equal(findByKey(catalogue, 'symbol', keyOf, 'Platinum'), null);
});

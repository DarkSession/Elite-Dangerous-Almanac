...
const prefixRunLength = (key: string): number => PREFIX_RUN_LENGTHS.get(key) ?? PREFIX_DEFAULT_RUN_LENGTH;
const infixRunLength = (key: string): number => INFIX_RUN_LENGTHS.get(key) ?? PREFIX_DEFAULT_RUN_LENGTH;
...
export function sectorNameFromCoords(coords: SectorCoords): string {
    ...
    if (name === null) {
        throw new RangeError(
            `Sector coordinate has no procedural name: ${JSON.stringify(coords)}`,
        );
    }
    return name;
}
...
export function sectorCoordsFromName(name: string): SectorCoords | null {
    ...
    for (const f of candidates) {
        ...
        if (
            f.length === 4 &&
            f[0]!.isPrefix && f[1]!.isSuffix && f[2]!.isPrefix && f[3]!.isSuffix
        ) {
            coords = c2SectorCoords(f);
        } else if (
            f.length === 3 &&
            f[0]!.isPrefix && f[1]!.isInfix && f[2]!.isSuffix
        ) {
            coords = c1SectorCoords3(f);
        } else if (
            f.length === 4 &&
            f[0]!.isPrefix && f[1]!.isInfix && f[2]!.isInfix && f[3]!.isSuffix
        ) {

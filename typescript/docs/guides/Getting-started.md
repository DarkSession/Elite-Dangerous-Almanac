...
before any transport compression — the published package strips whitespace but keeps
every identifier and expression as written, so its own files on disk are larger. The
heaviest imports are all deliberate:
...
They are also how you get line numbers. The published JavaScript keeps its function and
variable names, so an unmapped stack trace still names the frame that threw, but the
inlined catalogues make indentation a quarter of the package's weight and the build drops
it — run Node with `--enable-source-maps` and a throw resolves to its original
`src/**/*.ts` line and column instead of a column offset in a long generated line.

The maps cost about 2.4 MiB installed — roughly half of the unpacked package — while the
complete compressed npm archive remains about 610 KiB. They contain mappings and original
